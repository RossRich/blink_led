...
const uint main_loop_rate_ms = uint(1000.f / LOOP_RATE);
...
class Controller {
  class FSM {
    ...
    void transition(fsm_state_t new_state) {
      ...
      if (cur_trs != _states.end()) std::get<FN_IDX>(*cur_trs).on_exit();
      trs.on_enter();
      fsm_loop = &trs.loop;
  ...
  public:
    ...
    void loop() {
      if (current_state == fsm_state_t::TRANSITION) return;

      if (fsm_loop) (*fsm_loop)();
    }
  };
...
public:
  ...
  bool init() {
    ...
    if (res) _fsm.transition(fsm_state_t::START);
    return res;
  }
  ...
  void start() {
    ...
    if (_model->is_connection_ok() and _model->get_found().size()) {
      for (auto &id : _model->get_found()) {
        ...
        if (i.is_updated) _fsm.transition(fsm_state_t::WELL);
      }
    }
  }
  ...
  void well() {
    if (not _model->is_connection_ok()) _fsm.transition(fsm_state_t::ERROR);
  }

  void error_enter() { _view->connection_lost(); }

  void error() {
    if (_model->is_connection_ok()) _fsm.transition(fsm_state_t::WELL);
  }

  void run() {
    _radio->try_read();
    if (not time_reached(_start_time)) return;
    _start_time = make_timeout_time_ms(main_loop_rate_ms);
